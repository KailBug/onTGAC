...
class AgentState(TypedDict):
    ...
    schema: str  # 检索到的Schema,rerank后更新
    knowledge: str  #相关知识,初始化数据
    thinking: NotRequired[str]  # 当前思考
    action: str  # 最近执行动作(generate_sql/execute_sql/fix_sql),node中更新
    sql_state: str #success 或者 error,执行时更新
    current_sql: str  # 生成的SQL,refiner时更新
    error_msg: str #执行错误信息,执行时更新
    execution_result: NotRequired[Dict] # 执行结果
    error_count: int # 错误次数，当错误次数为3时停止,执行时更新
    final_sql: str  # 最终SQL,执行时更新
